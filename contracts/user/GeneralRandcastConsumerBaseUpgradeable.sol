...
/**
 * @notice Upgradeable version of {GeneralRandcastConsumerBase}.
 * @dev Provides callbackGasLimit auto-calculating and balance checking to save user's effort.
 *
 * Concrete contracts should:
 * 1. Set `adapter` in the constructor via {BasicRandcastConsumerBase} and call `_disableInitializers()`.
 * 2. Call `__GeneralRandcastConsumerBase_init()` from their `initialize` function.
 * 3. Inherit {UUPSUpgradeable} and implement `_authorizeUpgrade` if UUPS upgrades are desired.
 */
abstract contract GeneralRandcastConsumerBaseUpgradeable is
    ...
    OwnableUpgradeable
{
    ...
    function __GeneralRandcastConsumerBase_init() internal onlyInitializing {
        __Ownable_init();
    }
    ...
    /**
     * @dev Reserved storage gap for future upgrades. Do not remove or reorder existing state variables.
     */
